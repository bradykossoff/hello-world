...
Steps 1, 2, 4, 5 are done — all in code, no Studio modeler:

- **Models** are generated by `scripts/build-stages.luau` (Lune) into
  `src/serverstorage/RestaurantStages/Stage{1,2,3}.rbxmx`, Rojo-mapped to
  `ServerStorage.RestaurantStages`. Footprints landed at
  **24×24 / 32×28 / 40×32** (width×depth) rather than §3's 24/34/44 wide —
  the extra width overhung the 50×50 plot floors. `scripts/check-stages.luau`
  asserts the §4 contract on every build.
- **Local frame deviation:** `Base` (PrimaryPart) sits at the *entrance
  threshold*, not the footprint centre. Street is local **+X**, kitchen
  local **−X**, tables spread along local **Z**. This keeps the entrance in
  one spot across all three stages (§3's stated goal) — the building grows
  backward and sideways. `EntranceMarker.LookVector` = local +X.
- **`plot.BuildAnchor`** (added by `scripts/patch-place.luau`) sits 15 studs
  street-ward of each plot-floor centre; right-hand plots (2/4/6) are yawed
  180° so local +X resolves to world −X (toward the hub). Old
  `Stage1Building` models were deleted, not renamed. Spawn pads nudged to
  match.
- **Still to do:** step 3 (art replacing graybox parts), step 6 (per-roof
